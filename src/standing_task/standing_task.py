...
from utils.orientation_cost_function import orientation_cost
...
class StandingTelosTask:
    ...
    def is_terminated(self, agent_pos: np.ndarray) -> bool:
        max_angle_diff = np.max(
            np.abs(
                self._agent_starting_orientation
                - self.sim.get_orientation(self.agent.robot_agent)
            )
        )
        is_terminated = (
        ...
        if np.linalg.norm(agent_pos - self.goal) < self.dist_threshold:
    ...
    def compute_reward(
        ...
        smoothing_reward = -self.smoothing_factor * abs(
            np.sum(self.sim.get_velocity_from_rotary(self.agent.robot_agent))
        )

        orientation_cost_reward = -self.angle_dip_bias * abs(
            # math.pow(
            orientation_cost(
                current_orientation=self.sim.get_orientation(self.agent.robot_agent),
